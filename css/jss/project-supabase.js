(() => {
  ...
  async function init(){
    ...
    if(admin){
      ...
      form.addEventListener('submit',async e=>{e.preventDefault();const now=new Date().toISOString();p.deadline=deadline.value;p.stages.forEach((s,i)=>{const done=stages.querySelector(`[data-stage-index="${i}"]`).checked;const name=stages.querySelector(`[data-stage-name="${i}"]`).value.trim()||s.name;if(done!==s.done)p.history.unshift({id:crypto.randomUUID(),text:done?`Etapa “${name}” concluída.`:`Etapa “${name}” reaberta.`,at:now});s.done=done;s.name=name;s.doneAt=done?(s.doneAt||now):null;});p.updatedAt=now;const pct=progress(p);const status=pct===100?'Concluída':(item.status==='Aprovada'||item.status==='Em andamento'||item.status==='Concluída'?'Em andamento':item.status);const {error}=await db.from('requests').update({project:p,status}).eq('id',item.id);item.project=p;item.status=status;if(error){feedback.textContent='Não foi possível salvar o andamento.';feedback.className='form-message error';return;}const u=(await db.auth.getUser()).data.user;if(u)await db.from('request_events').insert({request_id:item.id,actor_id:u.id,actor_role:'admin',event_type:'project_progress',title:'Andamento do projeto atualizado',description:`${pct}% concluído · previsão ${fmtDate(p.deadline)}`,read_by_admin:true,read_by_client:false});feedback.textContent='Andamento salvo e disponibilizado para o cliente.';feedback.className='form-message success';render();renderHistory(document.getElementById('admin-project-history'),item,p);});
    }
