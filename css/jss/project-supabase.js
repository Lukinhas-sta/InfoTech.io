(() => {
  ...
  async function init(){
    ...
    if(admin){
      ...
      form.addEventListener('submit',async e=>{e.preventDefault();const now=new Date().toISOString();p.deadline=deadline.value;p.stages.forEach((s,i)=>{const done=stages.querySelector(`[data-stage-index="${i}"]`).checked;const name=stages.querySelector(`[data-stage-name="${i}"]`).value.trim()||s.name;if(done!==s.done)p.history.unshift({id:crypto.randomUUID(),text:done?`Etapa “${name}” concluída.`:`Etapa “${name}” reaberta.`,at:now});s.done=done;s.name=name;s.doneAt=done?(s.doneAt||now):null;});p.updatedAt=now;const pct=progress(p);const status=pct===100?'Concluída':(item.status==='Aprovada'||item.status==='Em andamento'||item.status==='Concluída'?'Em andamento':item.status);feedback.textContent='Salvando andamento...';feedback.className='form-message';const {data:saved,error:saveError}=await db.rpc('admin_update_request_project',{p_request_id:item.id,p_project:p,p_status:status});if(saveError||!saved){console.error('Falha ao salvar andamento:',saveError);feedback.textContent=saveError?.message||'Não foi possível salvar o andamento. Execute o SQL de correção no Supabase.';feedback.className='form-message error';return;}item.project=saved.project||p;item.status=saved.status||status;const confirmed=normalize(item.project);p.deadline=confirmed.deadline;p.stages=confirmed.stages;p.history=confirmed.history;p.updatedAt=confirmed.updatedAt;const u=(await db.auth.getUser()).data.user;if(u)await db.from('request_events').insert({request_id:item.id,actor_id:u.id,actor_role:'admin',event_type:'project_progress',title:'Andamento do projeto atualizado',description:`${progress(p)}% concluído · previsão ${fmtDate(p.deadline)}`,read_by_admin:true,read_by_client:false});feedback.textContent='Andamento salvo e disponibilizado para o cliente.';feedback.className='form-message success';render();renderHistory(document.getElementById('admin-project-history'),item,p);});
    }
