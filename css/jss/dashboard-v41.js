(() => {
  ...
  const insertDashboardDetails = (content, items, admin=false) => {
    ...
    block.innerHTML=`<section class="activity-card"><div class="activity-card-head"><div><span class="eyebrow">Movimentações</span><h2>Atividades recentes</h2></div></div>${activities.length?`<ol class="activity-list">${activities.map(a=>`<li class="activity-item"><span class="activity-dot">${icon(a.icon)}</span><span class="activity-copy"><strong>${escapeHtml(a.title)}</strong><p>${escapeHtml(a.text)}</p><time>${formatDate(a.date)}</time></span></li>`).join('')}</ol>`:'<div class="activity-empty">As atividades aparecerão aqui conforme o sistema for usado.</div>'}</section><section class="quick-overview-card"><span class="eyebrow">Distribuição</span><h2>Visão dos atendimentos</h2><div class="overview-bars"><div><div class="overview-row-head"><span>Em análise</span><strong>${analysis}</strong></div><div class="overview-track"><span class="overview-fill yellow" style="width:${analysis/total*100}%"></span></div></div><div><div class="overview-row-head"><span>Em andamento</span><strong>${progress}</strong></div><div class="overview-track"><span class="overview-fill" style="width:${progress/total*100}%"></span></div></div><div><div class="overview-row-head"><span>Concluídas</span><strong>${done}</strong></div><div class="overview-track"><span class="overview-fill green" style="width:${done/total*100}%"></span></div></div></div><p class="dashboard-mini-note">Os indicadores são atualizados automaticamente com os dados de demonstração salvos na sua conta.</p></section>`;
