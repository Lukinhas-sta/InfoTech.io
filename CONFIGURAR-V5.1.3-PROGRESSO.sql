-- Infotech.io 5.1.3 — Correção definitiva do salvamento do andamento
...
begin
  ...
  if v_request_id is null or not exists(select 1 from public.requests where id=v_request_id) then
  ...
  if jsonb_array_length(v_stages)=0 then
    ...
    select round(100.0 * count(*) filter(where coalesce((s->>'done')::boolean,false)) / count(*))::integer
      into v_progress from jsonb_array_elements(v_stages) s;
  end if;

  insert into public.request_projects(request_id,deadline,stages,history,progress,updated_at,updated_by)
  values(v_request_id,v_deadline,v_stages,v_history,v_progress,v_updated,auth.uid())
  ...
  update public.requests set
    project=jsonb_build_object('deadline',coalesce(v_deadline::text,''),'stages',v_stages,'history',v_history,'updatedAt',v_updated),
    status=v_status,
  ...
  from public.request_projects rp join public.requests r on r.id=rp.request_id
